...
Controllers live under **`com.jakartaee.jobfinder.servlet`**. Add CDI beans under **`com.jakartaee.jobfinder.service`** as logic moves out of servlets.

```
src/main/java/com/jakartaee/jobfinder/
├── servlet/         # @WebServlet entry points
└── service/         # AuthService, UserService, JobService, … (planned)
